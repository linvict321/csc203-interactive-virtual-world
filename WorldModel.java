    ...
    */
    ...
    private void parseSapling(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Sapling.SAPLING_NUM_PROPERTIES) {
            ...
            Sapling sapling = Sapling.createSapling(id, pt, imageStore.getImageList(Sapling.SAPLING_KEY), health);
    ...
    private void parseDude(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == DudeNotFull.DUDE_NUM_PROPERTIES) {
            DudeNotFull dude = DudeNotFull.createDudeNotFull(id, pt, Double.parseDouble(properties[DudeNotFull.DUDE_ACTION_PERIOD_IDX]), Double.parseDouble(properties[DudeNotFull.DUDE_ANIMATION_PERIOD_IDX]), Integer.parseInt(properties[DudeNotFull.DUDE_RESOURCE_LIMIT_IDX]), imageStore.getImageList(DudeNotFull.DUDE_KEY));
    ...
    private void parseFairy(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Fairy.FAIRY_NUM_PROPERTIES) {
            Fairy fairy = Fairy.createFairy(id, pt, imageStore.getImageList(Fairy.FAIRY_KEY), Double.parseDouble(properties[Fairy.FAIRY_ACTION_PERIOD_IDX]), Double.parseDouble(properties[Fairy.FAIRY_ANIMATION_PERIOD_IDX]));
    ...
    private void parseTree(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Tree.TREE_NUM_PROPERTIES) {
            Tree tree = Tree.createTree(id, pt, Double.parseDouble(properties[Tree.TREE_ACTION_PERIOD_IDX]), Double.parseDouble(properties[Tree.TREE_ANIMATION_PERIOD_IDX]), Integer.parseInt(properties[Tree.TREE_HEALTH_IDX]), imageStore.getImageList(Tree.TREE_KEY));
    ...
    private void parseObstacle(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Obstacle.OBSTACLE_NUM_PROPERTIES) {
            Obstacle obstacle = Obstacle.createObstacle(id, pt, Double.parseDouble(properties[Obstacle.OBSTACLE_ANIMATION_PERIOD_IDX]), imageStore.getImageList(Obstacle.OBSTACLE_KEY));
    ...
    private void parseHouse(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == House.HOUSE_NUM_PROPERTIES) {
            House house = House.createHouse(id, pt, imageStore.getImageList(House.HOUSE_KEY));
    ...
    private void parseStump(String[] properties, Point pt, String id, ImageStore imageStore) {
        if (properties.length == Stump.STUMP_NUM_PROPERTIES) {
            Stump stump = Stump.createStump(id, pt, imageStore.getImageList(Stump.STUMP_KEY));
    ...
    public void parseEntity(String line, ImageStore imageStore) {
        ...
        if (properties.length >= ENTITY_NUM_PROPERTIES) {
            ...
            switch (key) {
                ...
                case DudeNotFull.DUDE_KEY -> this.parseDude(properties, pt, id, imageStore);
